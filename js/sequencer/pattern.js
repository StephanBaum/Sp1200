...
class Track {
  ...
  removeEventAtTick(tick) { const idx = this.events.findIndex(e => e.tick === tick); if (idx !== -1) this.events.splice(idx, 1); }
  clear() { this.events = []; }
...
export class Pattern {
  ...
  setBars(bars) { this.bars = Math.max(1, Math.min(4, bars)); this.totalTicks = PPQN * 4 * this.bars; }
  addEvent(trackIndex, event) { if (trackIndex >= 0 && trackIndex < NUM_PADS) this.tracks[trackIndex].addEvent(event); }
  removeEvent(trackIndex, tick) { if (trackIndex >= 0 && trackIndex < NUM_PADS) this.tracks[trackIndex].removeEventAtTick(tick); }
  ...
  clearTrack(trackIndex) { if (trackIndex >= 0 && trackIndex < NUM_PADS) this.tracks[trackIndex].clear(); }
