...
import { NUM_PADS, MAX_PATTERNS, BASE_PITCH_STEP } from '../../js/constants.js';
...
describe('SetupHandler', () => {
  ...
  it('copy-sound copies buffer from one voice to another', () => {
    // Clear voice 5 sample first
    proc.voices[5].sample = null;
    handler.handle({ type: 'copy-sound', from: 0, to: 5 });
    expect(proc.voices[5].sample).not.toBeNull();
    expect(proc.voices[5].sample.length).toBe(100);
  ...
  it('copy-sound handles null source', () => {
    proc.voices[2].sample = null;
    handler.handle({ type: 'copy-sound', from: 2, to: 4 });
    expect(proc.voices[4].sample).toBeNull();
  });

  // ── swap-sounds ──────────────────────────────────────────────────────────

  it('swap-sounds swaps buffers between two voices', () => {
    const sampleA = new Float32Array([1, 2, 3]);
    const sampleB = new Float32Array([4, 5, 6, 7]);
    proc.voices[0].loadSample(sampleA);
    proc.voices[1].loadSample(sampleB);
